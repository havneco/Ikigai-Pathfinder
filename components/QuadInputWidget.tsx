...
const QuadInputWidget: React.FC<QuadInputWidgetProps> = ({ data, onUpdate, onRegenerate, isAnalysing, compact = false }) => {
  const [activeTab, setActiveTab] = useState<'love' | 'goodAt' | 'worldNeeds' | 'paidFor'>('love');
  const [inputValue, setInputValue] = useState('');
  const [isExpanded, setIsExpanded] = useState(!compact); // Auto-collapse if compact mode
  ...
  return (
    <div className={`flex flex-col bg-white rounded-3xl shadow-lg border border-slate-200 overflow-hidden transition-all duration-300 ${compact ? (isExpanded ? 'h-[400px]' : 'h-auto') : 'h-full'}`}>
      {/* Header Tabs (Compact: Click to Expand) */}
      <div className="flex border-b border-slate-100 overflow-x-auto no-scrollbar relative">
        {compact && (
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="absolute right-2 top-2 z-10 p-1 bg-slate-100 rounded-full hover:bg-slate-200 text-slate-500"
          >
            {isExpanded ? <X size={14} /> : <Plus size={14} className="rotate-45" />}
          </button>
        )}
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              key={tab.id}
              onClick={() => { setActiveTab(tab.id as any); setIsExpanded(true); }}
              className={`flex-1 min-w-[80px] ${compact ? 'py-2' : 'py-3'} flex flex-col items-center gap-1 transition-colors border-b-2 ${isActive ? `border-${tab.color.split('-')[1]}-500 ${tab.bg}` : 'border-transparent hover:bg-slate-50'
                }`}
            >
              <Icon size={compact ? 14 : 16} className={isActive ? tab.color : 'text-slate-400'} />
      ...
      {/* Editor Area (Hidden if Collapsed in Compact Mode) */}
      {(!compact || isExpanded) && (
