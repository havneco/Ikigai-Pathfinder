  ...
  const performAnalysis = async () => {
    ...
    try {
      ...
      const enrichedIdeas = initialIdeas.map(idea => ({
        ...
        score: idea.score || { total: 0, demand: 0, profit: 0, talent: 0, complexity: 0, passion: 0 },
        validation: {
          whyNow: (idea as any).whyNow || "Analyzing...",
          marketGap: "Analyzing...",
    ...
    <div className={`min-h-screen w-full bg-gradient-to-br ${getBgClass()} transition-colors duration-1000 flex flex-col font-sans text-slate-800`}>
      ...
      <main className="flex-1 container mx-auto px-4 py-8 flex flex-col justify-center items-center relative overflow-hidden">
        {step === Step.WELCOME && (
          <>
            {/* BACKGROUND HERO */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-[20%] -z-10 pointer-events-none select-none opacity-40 origin-top">
